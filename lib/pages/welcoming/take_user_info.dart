        ...
        body: Stack(
          children: [
            ...
            Align(
              ...
              child: Image.asset("assets/images/manOn_stance.jpg",
                fit: BoxFit.cover,
              ),
            ),
            Container(
                ...
                children: [
                    Row(
                      children: [
                        ...
                        Text("FitLife",
                          style: TextStyle(color: mainGreenGenerator(1),
                  ...
                  SizedBox(height: 20,),
                  Text("Fill the following to get Started!  ",
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      errorText: (appProvider.isUserNameSet)?"*Please Enter userName!":null,
                      hintText: "User Name",
                  ...
                  Align(
                    ...
                    child: (!appProvider.ageNotSet)?Text("  *Please Enter birth date!",
                    ...
                    if(appProvider.checkUserProfileData()){
                      ...
                      Navigator.push(context,
                          MaterialPageRoute(builder: (_)=>
                              popUPAlert(context,
                                  "Data", ["Saved successfully!", "Not Saved!"],
                                  [MainPage(), TakeRegistry()], true),
                          )
                      );
                    }
                    appProvider.changeUserNameError(appProvider.userName == "");
...
class _GenderSelection extends State<GenderSelection>{
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox( width: screenSize.width/2.5,
      child: Column(
        children: [
          RadioListTile<String>(title: Text("Male",
            style: TextStyle(
                color: (_selectedGender == "male")?mainWhiteGenerator(0.85) : mainGreyGenerator(1),
            ),
          ),
            value: "male",
          ...
          RadioListTile<String>(title: Text("Female",
          style: TextStyle(
              color: (_selectedGender == "female")?mainWhiteGenerator(0.85): mainGreyGenerator(1)),
          ),
            value: "female",
            groupValue: _selectedGender,
            onChanged: (value){
...
class _SliderSelection extends State<SliderSelection>{
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text("Weight $_userWeight kg",
          style: TextStyle(color: mainWhiteGenerator(0.8)),
        ),
        Slider(value: _userWeight,
        ...
        Text("Height $_userHeight cm",
          style: TextStyle(color: mainWhiteGenerator(0.8)),
        ),
        Slider(value: _userHeight,
            thumbColor: mainGreenGenerator(1),
            activeColor: mainGreenGenerator(0.5),
            divisions: 200,
            label: "Height",
...
class _DatePickerCalender extends State<DatePickerCalender>{
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Text("Age  ${(agePicked != null)? DateTime.now().year - agePicked!.year: "not Set"} ",
          style: TextStyle(color: mainWhiteGenerator(1)),
        ),
        SizedBox(width: 20,),
        ElevatedButton(onPressed: () async{
          agePicked = await showDatePicker(context: context,
            initialDatePickerMode: DatePickerMode.year,
            firstDate: DateTime(1945),
            lastDate: DateTime(2015, DateTime.december, 31),
            switchToInputEntryModeIcon: Icon(CupertinoIcons.keyboard,
                color: mainGreenGenerator(1)),
            switchToCalendarEntryModeIcon: Icon(CupertinoIcons.calendar_circle_fill,
              color: mainGreenGenerator(1),),errorInvalidText: "Please input a valid date",
            barrierColor: mainGreenGenerator(0.37),
            barrierLabel: "Choose your birth date",
          );
          setState(() {
            userProfileInfo["age"] = (agePicked != null)? DateTime.now().year - agePicked!.year: -1;
            appProvider.changeAgeSet(userProfileInfo["age"] != -1);
          });
        }, child: Icon(CupertinoIcons.calendar_badge_plus, color: mainBlackGenerator(1),),
        ),
      ],
    );
  }
